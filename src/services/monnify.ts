...
export async function verifyTransaction(reference: string) {
  ...
  if (reference.toUpperCase().startsWith('MNFY|')) {
    console.log(`Trying v2 transaction status endpoint`)
    const v2TransactionResult = await tryWithRetry(async () => {
      const url = `${BASE}/api/v2/transactions/${encodeURIComponent(reference)}`
      return queryUrl(url)
    })
    if (v2TransactionResult) return v2TransactionResult
  }

  // Strategy 1a: Query by merchant paymentReference. This is the exact reference
  // generated by our app before Monnify later returns its MNFY transactionReference.
  console.log(`Trying v2 merchant transaction query by paymentReference`)
  const merchantPaymentReferenceResult = await tryWithRetry(async () => {
    const url = `${BASE}/api/v2/merchant/transactions/query?paymentReference=${encodeURIComponent(reference)}`
    return queryUrl(url)
  })
  if (merchantPaymentReferenceResult) return merchantPaymentReferenceResult

  console.log(`Trying v2 merchant transaction query by transactionReference`)
  const merchantTransactionReferenceResult = await tryWithRetry(async () => {
    const url = `${BASE}/api/v2/merchant/transactions/query?transactionReference=${encodeURIComponent(reference)}`
    return queryUrl(url)
  })
  if (merchantTransactionReferenceResult) return merchantTransactionReferenceResult

  // Strategy 2: Try SDK transactions endpoint with contract code
  if (contractCode) {
  ...
  // Strategy 3: Try transactions search endpoint
  ...
  // Strategy 3b: fallback to merchant transactions endpoint for older accounts
  ...
  // Strategy 4: Try direct transaction query endpoints
  ...
  // Strategy 5: Retry SDK query and accept when Monnify returns the same payment under an alternate reference field
  if (contractCode) {
...
export async function refundTransaction({
  ...
  const body: Record<string, unknown> = {
    ...
    refundReference: refundReference || `refund-${Date.now()}`,
    refundReason: reason || 'Bill payment service failed',
  ...
  const res = await fetch(`${BASE}/api/v1/refunds/initiate-refund`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
