...
import { collection, query, where, onSnapshot, getDocs, doc, getDoc, limit } from "firebase/firestore"
...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      ...
      // --- Withdrawals ---
      const q2 = query(collection(db, "advertiserWithdrawals"), where("userId", "==", user.uid), limit(150))
      ...
      ;(async () => {
        try {
          const advQ = query(collection(db, 'advertisers'), where('email', '==', user.email), limit(1))
      ...
      try {
        const txQ = query(collection(db, 'advertiserTransactions'), where('userId', '==', user.uid), limit(250))
