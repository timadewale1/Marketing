...
export async function GET(request: Request) {
  ...
  const pushActivationTx = (snap: FirebaseFirestore.QuerySnapshot | null) => {
    if (!snap) return
  ...
  pushActivationTx(earnerActivationTxSnap)
  pushActivationTx(advertiserActivationTxSnap)
  ...
  const activationCandidates = (await Promise.all(
    [...earnersSnap.docs.map((doc) => ({ doc, role: "earner" as const })), ...advertisersSnap.docs.map((doc) => ({ doc, role: "advertiser" as const }))]
      .map(async ({ doc, role }) => {
        ...
        const verificationState = txInfo?.reference
          ? "paid"
          : await resolveRecoveryVerificationState(
              references,
              ...
              successfulWebhookReferences,
              {
                ...
                amount: 2000,
                notBefore: (serializeDate(data.activationAttemptedAt) as string | null) || attemptInfo?.activationAttemptedAt || txInfo?.createdAt || null,
              }
            )
  ...
  for (const candidate of activationCandidates) {
    ...
    try {
      ...
      console.error("[cron][recovery] activation recovery failed", { candidate, error })
    }
  }
  ...
  for (const candidate of walletCandidates) {
    ...
    try {
      ...
      console.error("[cron][recovery] wallet recovery failed", { candidate, error })
    }
  }
