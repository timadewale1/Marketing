...
export default function VendorBankPage() {
  ...
  const updateBankDetails = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, "vendors", u.uid), { bank: bankData });

      toast.success("Bank details updated successfully");
      setSelectedBank("");
      setAccountNumber("");
      setAccountName("");
      const fresh = await getDoc(doc(db, "vendors", u.uid));
      if (fresh.exists()) {
        setBank((fresh.data().bank as Bank) || null);
      }
    } catch (error) {
      console.error(error);
        ...
        {loading ? (
          ...
          <>
            <Card className="bg-white/80 backdrop-blur p-6 mb-6">
              ...
              {bank ? (
                <div className="space-y-2">
                  ...
                  <div className="pt-2">
                    <span className="inline-block px-2 py-1 text-xs font-medium bg-green-100 text-green-700 rounded-full">Verified</span>
                  </div>
                </div>
              ) : (
            ...
            <Card className="bg-white/80 backdrop-blur p-6">
              <h2 className="text-lg font-semibold text-stone-800 mb-4">{bank ? "Update Bank Account" : "Add Bank Account"}</h2>
              <div className="space-y-4">
                    ...
                    <PopoverContent className="w-[320px] max-h-80 overflow-hidden bg-white p-0 rounded-lg shadow-lg">
                      <Command>
                        ...
                        <CommandList className="max-h-72 overflow-y-auto">
                          ...
                          <CommandGroup>
                            {banks.map((item, idx) => (
                              <CommandItem key={`${item.code}-${idx}`} onSelect={() => setSelectedBank(item.code)}>
                                {item.name}
                              </CommandItem>
                            ))}
                          </CommandGroup>
                ...
                <div className="flex gap-3 pt-4">
                  <Button onClick={updateBankDetails} disabled={submitting || !accountName} className="flex-1 bg-cyan-700 hover:bg-cyan-600 text-white font-medium">
