...
export default function SubmissionManagementCampaignDetailClient({ id }: Props) {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (campaignSnap.exists()) {
          ...
          const paymentTx = transactionsSnap.docs.find((docItem) => docItem.data().type === "campaign_payment");
          const paymentData = paymentTx?.data() || {};
          campaignRow = {
            id,
            title: String(submissionRows[0]?.campaignTitle || paymentData.campaignTitle || "Deleted campaign"),
            description: "This campaign document no longer exists in the campaigns collection. The dashboard is reconstructing it from submissions and transactions.",
            proofInstructions: "",
  ...
  const summary = useMemo(() => summarizeCampaignProgress({
  ...
  const sendCampaignAction = async (action: "activate" | "pause" | "stop" | "delete") => {
    try {
      ...
      const response = await fetch("/api/submissionmanagement/campaign", {
        method: "POST",
        credentials: "include",
      ...
      if (!response.ok || !data.success) throw new Error(data.message || "Campaign action failed");
      setCampaign((current) => current ? { ...current, status: action === "activate" ? "Active" : action === "pause" ? "Paused" : action === "stop" ? "Stopped" : "Deleted" } : current);
  ...
  if (loading) return <div className="h-64 animate-pulse rounded-3xl bg-stone-100" />;

  if (!campaign) {
    return (
      <div className="space-y-6">
        <AdminPageHeader eyebrow="Campaign detail" title="Campaign not found" description="This id does not exist in campaigns and no related submissions or transactions were found." />
        <EmptyState title="No campaign data" description="The campaign may never have been created in this Firebase project, or every related record may already have been cleaned up." href="/submissionmanagement/campaigns" cta="Back to campaigns" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8">
      <AdminPageHeader
        ...
        description="Campaign context, proof queue, and transaction history are available here without the advertiser profile card."
        action={
          <div className="flex flex-wrap gap-2">
            {campaign.status !== "Active" && campaign.status !== "Deleted" ? <Button variant="outline" className="rounded-full border-emerald-200 text-emerald-700 hover:bg-emerald-50" disabled={Boolean(actionLoading)} onClick={() => sendCampaignAction("activate")}><PlayCircle className="h-4 w-4" />Activate</Button> : null}
            {campaign.status === "Active" ? <Button variant="outline" className="rounded-full border-amber-200 text-amber-700 hover:bg-amber-50" disabled={Boolean(actionLoading)} onClick={() => sendCampaignAction("pause")}><PauseCircle className="h-4 w-4" />Pause</Button> : null}
            {campaign.status !== "Stopped" && campaign.status !== "Deleted" ? <Button variant="outline" className="rounded-full border-rose-200 text-rose-700 hover:bg-rose-50" disabled={Boolean(actionLoading)} onClick={() => sendCampaignAction("stop")}><SquareSlash className="h-4 w-4" />Stop</Button> : null}
            {campaign.status !== "Deleted" ? <Button variant="outline" className="rounded-full border-rose-200 text-rose-700 hover:bg-rose-50" disabled={Boolean(actionLoading)} onClick={() => sendCampaignAction("delete")}><Trash2 className="h-4 w-4" />Delete</Button> : null}
          </div>
        }
      />

      <div className="flex flex-wrap gap-2">
        <StatusBadge label={campaign.status} tone={campaign.status === "Active" ? "green" : campaign.status === "Paused" ? "amber" : campaign.status === "Deleted" ? "red" : "blue"} />
        <StatusBadge label={campaign.category} tone="stone" />
        {campaign.deletedFallback ? <StatusBadge label="Reconstructed from related records" tone="amber" /> : null}
      </div>
      ...
      {campaign.deletedFallback ? (
        <div className="rounded-3xl border border-amber-200 bg-amber-50/90 p-4 text-sm text-amber-900">
          <div className="flex items-start gap-3">
            ...
            <p>The campaign document has already been removed from Firestore, but submissions and transactions still reference it. Pending submissions can still be rejected, but verification is blocked because the source campaign no longer exists.</p>
          </div>
        </div>
      ) : null}

      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard label="Available budget" value={currency(campaign.budget)} hint={`${currency(campaign.reservedBudget)} reserved`} icon={Wallet} />
        <MetricCard label="Original budget" value={currency(campaign.originalBudget || campaign.budget + campaign.reservedBudget)} hint={`${currency(campaign.earnerPrice)} per proof`} icon={Wallet} tone="blue" />
        <MetricCard label="Submissions" value={summary.totalSubmissions} hint={`${summary.pending} pending review`} icon={UserRound} tone="amber" />
        <MetricCard label="Verified" value={summary.verified} hint={`${summary.rejected} rejected`} icon={CheckCheck} tone="emerald" />
      </div>

      <SectionCard title="Campaign overview" description="High-level campaign metadata, instructions, and lead progress.">
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-2xl bg-stone-50 p-4">
            <p className="text-xs uppercase tracking-[0.24em] text-stone-500">Created</p>
            <p className="mt-3 text-sm text-stone-700">{campaign.createdAtMs ? new Date(campaign.createdAtMs).toLocaleString() : "Unknown"}</p>
          </div>
          <div className="rounded-2xl bg-stone-50 p-4">
            <p className="text-xs uppercase tracking-[0.24em] text-stone-500">Leads</p>
            <p className="mt-3 text-sm text-stone-700">{summary.verified} verified{summary.target ? ` / ${summary.target} target` : ""}</p>
            <p className="mt-2 text-xs text-stone-500">{summary.pending} pending review • {summary.rejected} rejected</p>
          </div>
          <div className="rounded-2xl bg-stone-50 p-4 md:col-span-2">
            ...
            <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-stone-700">{campaign.description || "No campaign description recorded."}</p>
          </div>
          <div className="rounded-2xl bg-stone-50 p-4 md:col-span-2">
      ...
      <SectionCard title="Proof queue" description="Review every submission from this campaign here, including the proof link and direct approve/reject controls.">
        {submissions.length === 0 ? (
          <EmptyState title="No submissions yet" description="No earner has submitted proof for this campaign." />
        ) : (
          <PaginatedCardList
            items={submissions}
            itemsPerPage={3}
            renderItem={(submission) => (
              <div key={submission.id} className="rounded-2xl border border-stone-200 bg-white p-4">
                <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                  <div className="space-y-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <p className="font-medium text-stone-900">Earner {submission.userId}</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {submission.proofUrls.map((proof, index) => (
                      <Button key={`${submission.id}-proof-${index}`} asChild variant="outline" className="rounded-full">
                        <Link href={proof} target="_blank">View proof {index + 1}<ExternalLink className="h-4 w-4" /></Link>
                      </Button>
                    ))}
                    {submission.status !== "Verified" ? <Button className="rounded-full bg-emerald-600 text-white hover:bg-emerald-700" disabled={Boolean(actionLoading) || campaign.deletedFallback} onClick={() => reviewSubmission(submission, "Verified")}><CheckCheck className="h-4 w-4" />{submission.status === "Rejected" ? "Re-verify" : "Verify"}</Button> : null}
                    {submission.status !== "Rejected" ? <Button variant="outline" className="rounded-full border-rose-200 text-rose-700 hover:bg-rose-50" disabled={Boolean(actionLoading)} onClick={() => reviewSubmission(submission, "Rejected")}><XCircle className="h-4 w-4" />Reject</Button> : null}
                  </div>
                </div>
              </div>
            )}
      ...
      <SectionCard title="Transaction history" description="All advertiser-side payment and refund events currently tied to this campaign.">
        {transactions.length === 0 ? (
          <EmptyState title="No transactions" description="No advertiser transaction records were found for this campaign." />
        ) : (
          <PaginatedCardList
            items={transactions}
            itemsPerPage={3}
            renderItem={(transaction) => (
              <div key={transaction.id} className="rounded-2xl border border-stone-200 bg-white p-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-medium capitalize text-stone-900">{transaction.type.replace(/_/g, " ")}</p>
                    <p className="mt-1 text-sm text-stone-500">{transaction.note || "No note"}</p>
                    <p className="mt-2 text-xs uppercase tracking-[0.24em] text-stone-400">{transaction.createdAtMs ? new Date(transaction.createdAtMs).toLocaleString() : "Unknown date"}</p>
                  </div>
                  <div className="text-right">
                    <p className={`text-lg font-semibold ${transaction.amount >= 0 ? "text-emerald-700" : "text-rose-700"}`}>{currency(transaction.amount)}</p>
