...
export default function EducationPage() {
    ...
    ;(async () => {
      try {
        const [wRes, jRes] = await Promise.all([
          fetch('/api/vtpass/variations?serviceID=waec'),
          fetch('/api/vtpass/variations?serviceID=jamb'),
        ])
  ...
  const verifyJamb = async () => {
    if (!jambProfile) return toast.error('Enter JAMB profile/registration')
    try {
      const res = await fetch('/api/vtpass/merchant-verify', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ serviceID: 'jamb', billersCode: jambProfile }) })
      const j = await res.json()
